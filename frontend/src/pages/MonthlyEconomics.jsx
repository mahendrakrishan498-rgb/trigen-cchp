...
export default function MonthlyEconomics() {
  ...
  return <>
    ...
    {result && <><section className="panel chart-panel"><h3>Monthly export revenue and import cost</h3><ResponsiveContainer width="100%" height={330}><BarChart data={rows}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Legend /><Bar dataKey="export_revenue_lkr" name="Export revenue LKR" fill="#14a879" /><Bar dataKey="import_cost_lkr" name="Import cost LKR" fill="#f04438" /></BarChart></ResponsiveContainer></section><section className="panel"><table className="data-table"><thead><tr><th>Month</th><th>Hotel Elec kWh</th><th>Cooling kWh</th><th>Heating kWh</th><th>Turbine kWh</th><th>Grid Export kWh</th><th>Export Revenue</th></tr></thead><tbody>{rows.map((r)=><tr key={r.month}><td>{r.month}</td><td>{money(r.hotel_electricity_kwh)}</td><td>{money(r.cooling_thermal_kwh)}</td><td>{money(r.heating_thermal_kwh)}</td><td>{money(r.turbine_electricity_kwh)}</td><td>{money(r.grid_export_kwh)}</td><td>{money(r.export_revenue_lkr)}</td></tr>)}</tbody></table></section></>}
  </>;
}
