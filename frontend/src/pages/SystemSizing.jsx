...
export default function SystemSizing() {
  ...
  return <>
    ...
    {result && <><div className="grid cards"><MetricCard label="Boiler/steam generator" value={result.system_sizing.boiler_tph} unit="ton/hr" /><MetricCard label="Steam turbine" value={result.system_sizing.turbine_kw} unit="kW" /><MetricCard label="Total chiller" value={result.system_sizing.absorption_chiller_rt} unit="RT" /><MetricCard label="Heat exchanger" value={result.system_sizing.heat_exchanger_kw} unit="kW" /><MetricCard label="CAPEX" value={moneyShort(result.system_sizing.capex_lkr)} unit="" /><MetricCard label="Annual biomass" value={result.fuel.annual_biomass_tonnes} unit="tonnes/y" /></div><div className="two-col"><section className="panel"><h3>CAPEX build-up</h3><ResultTable data={result.step03_capex} /></section><section className="panel"><h3>Energy balance</h3><ResultTable data={result.energy_balance} /><h3>Fuel</h3><ResultTable data={result.fuel} /></section></div></>}
  </>;
}
