...
export default function Comparison() {
  ...
  return <>
    ...
    <section className="panel"><h3>Saved comparison</h3><table className="data-table"><thead><tr><th>Metric</th><th>Website</th><th>Excel/RETScreen</th><th>Unit</th><th>Error %</th></tr></thead><tbody>{saved.map((r)=><tr key={r.id}><td>{r.metric}</td><td>{Number(r.website_value||0).toLocaleString()}</td><td>{Number(r.retscreen_value||0).toLocaleString()}</td><td>{r.unit}</td><td>{Number(r.error_percent||0).toFixed(2)}</td></tr>)}</tbody></table></section>
  </>;
}
