...
export default function Admin() {
  ...
  return <>
    ...
    <section className="panel"><h3>Equipment quotation database</h3><table className="data-table"><thead><tr><th>Item</th><th>Config</th><th>Rooms</th><th>Capacity</th><th>Cost LKR</th><th>Supplier</th><th></th></tr></thead><tbody>{equipment.map((e)=><tr key={e.id}><td>{e.item_name}</td><td>{e.config_type}</td><td>{e.room_capacity}</td><td>{e.capacity_value} {e.capacity_unit}</td><td>{moneyShort(e.cost_lkr)}</td><td>{e.supplier}</td><td><button className="danger" onClick={()=>deleteEq(e.id)}>Delete</button></td></tr>)}</tbody></table></section>
  </>;
}
