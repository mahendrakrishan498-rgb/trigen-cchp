...
function calculate(input = {}, settings = {}, equipmentRows = [], exportTariffs = EXPORT_TARIFFS, bmsSummary = null) {
  ...
  const exportTariffYear1 = settingsValue(input, settings, 'grid_export_tariff_lkr_kwh', exportTariffForYear(projectYear, exportTariffs));
  ...
  const selectedMainRt = nextCandidate(requiredMainRt, CHILLER_CANDIDATES_RT);
  const selectedBackupRt = nextCandidate(requiredBackupRt, CHILLER_CANDIDATES_RT);
  const totalChillerRt = selectedMainRt + selectedBackupRt;
  ...
  const potentialTurbineKw = processSteamFlowKgH * turbineYield * steamUtil;
  ...
  const annualGridImport = Math.max(0, annualElectricity - annualTurbineElectricity);
  const annualGridExport = Math.max(0, annualTurbineElectricity - annualElectricity);
  const annualProcessSteamUsedKg = annualHeatingDemand / (steamDh / 3600) + annualCoolingThermal / absorptionCop / (steamDh / 3600);
  const annualTurbineInletSteamKg = selectedTurbineSteamFlowKgH * 8760;
  ...
  const directEquipmentCapex = dualChillerCapex + turbineCapex + steamGeneratorCapex + integrationCapex + gridInterconnectionCapex;
  ...
  const year1NetSavings = totalAvoidedEnergyCost - projectNetOperatingCost;
  ...
  for (let y=0; y<=analysisPeriod; y+=1) {
    ...
    const exportTariff = y === 0 ? 0 : exportTariffForYear(projectYear + y - 1, exportTariffs);
    ...
    const benefits = avoidedElectricity + avoidedHeating + exportRevenue;
  ...
  return {
    ...
    cash_flow:cashFlow,
    sensitivity,
    export_tariff_schedule:exportTariffs.map((r)=>({ year:r.year, om_lkr_kwh:r.om, fuel_lkr_kwh:r.fuel, fixed_lkr_kwh:r.fixed, total_lkr_kwh:round(n(r.om)+n(r.fuel)+n(r.fixed),2) })),
    methodology_notes:[
